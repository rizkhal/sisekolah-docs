...
That will return current `coupons` applied for current `plans` or null if current tenant does't have `plans` or have `plans` but does't have `coupons`, the `current` method also accept `callable` parameter if you want to make some conditional
...
When you pass the argument, the first query is not called because we using `->when()` to only accept argument if it exists or default query if argument is null
...
In this provider, we use to create menu for user, we using [this](https://github.com/nedwors/navigator) to create menu navigator
